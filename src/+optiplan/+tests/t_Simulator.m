...
msg = optiplan.utils.run_in_caller(@() optiplan.Simulator([]));
optiplan.utils.assert_errmsg(msg, 'The first input must be a Planner object');
end
...
psim = optiplan.Simulator(planner);
% x0 must be correct
msg = optiplan.utils.run_in_caller(@() psim.run([], 1));
optiplan.utils.assert_errmsg(msg, 'x0 must be a 4x1 vector.');
% parameters must be set
assert(~isempty(psim.listMissing));
msg = optiplan.utils.run_in_caller(@() psim.run(zeros(4, 1), 1));
optiplan.utils.assert_errmsg(msg, 'There are missing values.');
end
...
T = evalc('psim.listMissing');
assert(length(T)==120);
msg = optiplan.utils.run_in_caller(@() psim.run(x0, Nsim));
optiplan.utils.assert_errmsg(msg, 'There are missing values.');
