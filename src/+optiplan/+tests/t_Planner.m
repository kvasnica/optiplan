...
err = optiplan.utils.run_in_caller(@() optiplan.Planner([]));
optiplan.utils.assert_errmsg(err, 'Not enough input arguments.');
end

function test_constructor2(testInput)
% second input must be either empty or Obstacle
agent = optiplan.LinearAgent('nx', 4, 'nu', 2, 'ny', 2, 'PredictionHorizon', 10);
obst = 1;
err = optiplan.utils.run_in_caller(@() optiplan.Planner(agent, obst));
optiplan.utils.assert_errmsg(err, 'The second input must be an Obstacle object.');
end

function test_constructor3(testInput)
% constructor must work with empty obstacles and no solver
agent = optiplan.tests.testAgent1();
obstacles = [];
p = optiplan.Planner(agent, obstacles);
end
...
agent = optiplan.tests.testAgent1();
obstacles = optiplan.Obstacle(agent);
p = optiplan.Planner(agent, obstacles, 'solver', 'gurobi');
end
...
agent = optiplan.tests.testAgent1();
obstacles = [optiplan.Obstacle(agent); optiplan.Obstacle(agent); optiplan.Obstacle(agent);];
p = optiplan.Planner(agent, obstacles, 'solver', 'gurobi');
...
msg = optiplan.utils.run_in_caller(@() p.readParameters());
optiplan.utils.assert_errmsg(msg, 'Parameter.Agent(1).Size.Value must be either a 2x1 or a 2x10 matrix.');
end
...
agent = optiplan.tests.testAgent2(); % no obstacles
obstacles = [];
p = optiplan.Planner(agent, obstacles, 'solver', 'gurobi');
x0 = zeros(4, 1);
...
obstacles = [];
a1 = optiplan.tests.testAgent1();
a2 = optiplan.tests.testAgent2();
p = optiplan.Planner(a1, obstacles, 'solver', 'gurobi');
x0 = zeros(4, 1);
...
a2.Y.Reference = yref;
obstacles = [];
p = optiplan.Planner(a2, obstacles, 'solver', 'gurobi');
x0 = zeros(4, 1);
X = x0;
U = [];
for i = 1:50
...
[agent, obstacles] = optiplan.tests.testAgent3(1);
...
agent.Y.Reference = [0; 0];
% planner
planner = optiplan.Planner(agent, obstacles);
...
nobst = 3;
[agent, obstacles] = optiplan.tests.testAgent3(nobst);
planner = optiplan.Planner(agent, obstacles);
