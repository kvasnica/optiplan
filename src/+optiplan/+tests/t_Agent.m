...
msg = optiplan.utils.run_in_caller('agent.N = 12');
optiplan.utils.assert_errmsg(msg, 'You cannot set the read-only property');
...
msg = optiplan.utils.run_in_caller(@() agent.optimize());
optiplan.utils.assert_errmsg(msg, 'Not enough input arguments.');

%% optimize() must not work if we have parameters
x0 = zeros(4, 1);
msg = optiplan.utils.run_in_caller(@() agent.optimize(x0));
optiplan.utils.assert_errmsg(msg, 'Only agents without parameters can be optimized here.');
...
msg = optiplan.utils.run_in_caller('agent.A.Value=1');
optiplan.utils.assert_errmsg(msg, 'Value must be a 4x4 vector/matrix.');
