classdef Planner < moantool.utils.OMPBaseClass
    ...
    methods
        ...
        function obj = Planner(agent, obstacles, varargin)
            ...
            narginchk(2, Inf);
            assert(isa(agent, 'moantool.Agent'), 'The first input must be an Agent object.');
			% parse inputs	
            ip = inputParser;
            ...
            % for time-varying constraints, these must be set as parameters
            if ~options.MixedInteger
                obj.Agent.Y.Min = 'parameter';
                obj.Agent.Y.Max = 'parameter';
            end
            obj.Agent.instantiate();
            if ~isempty(obstacles)
                assert(isa(obstacles, 'moantool.Obstacle'), 'The second input must be an Obstacle object.');
        ...
        function listParameters(obj)
            ...
            listParameters@moantool.utils.OMPBaseClass(obj, prefix);
        end
