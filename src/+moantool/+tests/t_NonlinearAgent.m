...
agent = moantool.NonlinearAgent('nx', 4, 'nu', 2, 'ny', 2, 'PredictionHorizon', 10);
...
msg = moantool.utils.run_in_caller('agent.StateEq = 1');
moantool.utils.assert_errmsg(msg, 'The value must be a function handle.');
% the function must take 3 inputs
h = @(x, u) eye(4)*x+ones(4, 2)*u;
msg = moantool.utils.run_in_caller('agent.StateEq = h');
moantool.utils.assert_errmsg(msg, 'The function must take 3 inputs: x(k), u(k), agent');
% correct function
...
agent = moantool.NonlinearAgent('nx', 4, 'nu', 2, 'ny', 2, 'PredictionHorizon', 10);
% input must be a function handle
msg = moantool.utils.run_in_caller('agent.OutputEq = 1');
moantool.utils.assert_errmsg(msg, 'The value must be a function handle.');
% the function must take 3 inputs
h = @(x, u) eye(4)*x+ones(4, 2)*u;
msg = moantool.utils.run_in_caller('agent.OutputEq = h');
moantool.utils.assert_errmsg(msg, 'The function must take 3 inputs: x(k), u(k), agent');
% correct function
h = @(x, u, a) eye(4)*x+ones(4, 2)*u;
agent.OutputEq = h;
...
agent = moantool.NonlinearAgent('nx', 4, 'nu', 2, 'ny', 2, 'PredictionHorizon', 10);
% input must be a function handle
msg = moantool.utils.run_in_caller('agent.ConstraintsFun = 1');
moantool.utils.assert_errmsg(msg, 'The value must be a function handle.');
% the function must take 4 inputs
h = @(x, u, y) x;
msg = moantool.utils.run_in_caller('agent.ConstraintsFun = h');
moantool.utils.assert_errmsg(msg, 'The function must take 4 inputs: X, U, Y, agent');
% correct function
h = @(x, u, y, a) x;
...
agent = moantool.NonlinearAgent('nx', 4, 'nu', 2, 'ny', 2, 'PredictionHorizon', 10);
% input must be a function handle
msg = moantool.utils.run_in_caller('agent.ObjectiveFun = 1');
moantool.utils.assert_errmsg(msg, 'The value must be a function handle.');
% the function must take 4 inputs
h = @(x, u, y) x;
msg = moantool.utils.run_in_caller('agent.ObjectiveFun = h');
moantool.utils.assert_errmsg(msg, 'The function must take 4 inputs: X, U, Y, agent');
% correct function
h = @(x, u, y, a) x;
...
data = moantool.LinearAgent.demo2Ddata(0.25);
agent = moantool.NonlinearAgent('nx', 4, 'nu', 2, 'ny', 2, 'PredictionHorizon', 10);
...
msg = moantool.utils.run_in_caller(@() agent.linearize());
moantool.utils.assert_errmsg(msg, 'Not enough input arguments.');
msg = moantool.utils.run_in_caller(@() agent.linearize(1));
moantool.utils.assert_errmsg(msg, 'Not enough input arguments.');
msg = moantool.utils.run_in_caller(@() agent.linearize(1, 2));
moantool.utils.assert_errmsg(msg, '"X" must have 4 row(s).');
msg = moantool.utils.run_in_caller(@() agent.linearize(zeros(4, 1), 2));
moantool.utils.assert_errmsg(msg, '"U" must have 2 row(s).');
...
% linearization point
agent = moantool.NonlinearAgent('nx', 4, 'nu', 2, 'ny', 3, 'PredictionHorizon', 10);
agent.StateEq = @(x, u, ~) x+[x(3); x(4); u(1)*sin(u(2)); u(1)*cos(u(2))];
