...
msg = moantool.utils.run_in_caller(@() moantool.Obstacle());
...
obst = moantool.Obstacle(testInput.TestData.agent);
assert(isa(obst, 'moantool.Obstacle'));
...
for i = 1:length(expected_signals)
    ...
    assert(isa(sig, 'moantool.AgentSignal'));
    assert(length(sig)==N);
...
% multiple obstacles
o1 = moantool.Obstacle(testInput.TestData.agent);
o1.instantiate();
o2 = moantool.Obstacle(testInput.TestData.agent);
o2.instantiate();
O = [o1 o2];
J = O.objective();
assert(J==0);
...
msg = moantool.utils.run_in_caller(@() obst.getOptimizerData());
moantool.utils.assert_errmsg(msg, 'Object must be instantiated.');
...
nobst = 3;
obst = moantool.Obstacle(testInput.TestData.agent, nobst);
assert(length(obst)==nobst);
for i = 1:nobst, assert(isa(obst(i), 'moantool.Obstacle')); end
