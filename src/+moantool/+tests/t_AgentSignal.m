...
msg = moantool.utils.run_in_caller(@() moantool.AgentSignal(1));
moantool.utils.assert_errmsg(msg, 'The first input must be a 1x2 vector.');
...
z = moantool.AgentSignal([n 1], m);
msg = moantool.utils.run_in_caller(@() z.uninstantiate);
moantool.utils.assert_errmsg(msg, 'Signal is not instantiated.');
...
msg = moantool.utils.run_in_caller(@() z.constraints);
moantool.utils.assert_errmsg(msg, 'Signal is not instantiated.');
...
z = AgentSignal([n, 1], m);
msg = moantool.utils.run_in_caller(@() z(1).objective);
moantool.utils.assert_errmsg(msg, 'Signal is not instantiated.');
...
msg = moantool.utils.run_in_caller('f.Value=ones(2*n, 1)');
moantool.utils.assert_errmsg(msg, sprintf('Value must be a %dx1', n));
...
assert(length(f)==m);
% setter must check dimensions
msg = moantool.utils.run_in_caller('f.Value=1');
moantool.utils.assert_errmsg(msg, sprintf('Value must be a %dx%d', n, 1));
