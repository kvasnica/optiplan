classdef FilterBehavior < moantool.utils.MPTUIHandle
			...
			if ~out && ~isempty(varargin)
				...
				if options.recursive
					...
					for i = 1:length(p)
						v = obj.(p{i});
						for j = 1:numel(v)
							if isa(v(j), 'moantool.utils.FilterBehavior')
...
% 			setup = feval(obj.filterFunction(filter), obj);
            setup = feval(['filter_' filter], obj);
			if ~isa(setup, 'moantool.utils.FilterSetup')
            ...
            if ~isempty(setup.incompatibleWith)
                % check compatibility
