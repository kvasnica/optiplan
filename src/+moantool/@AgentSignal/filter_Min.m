function filter = filter_Min(varargin)
...
out = obj.default_getVariables_handler('Min', [obj.Dim(1), 1, obj.N]);
...
out = obj.default_instantiate_handler('Min', [obj.Dim(1), 1, obj.N]);
...
out = obj.default_uninstantiate_handler('Min');
...
out = [];
if isnumeric(obj.Min)
    % fixed bounds
    for i = 1:obj.Dim(1)
        % Do not include +/-Inf bounds
        if ~isinf(obj.Min(i))
            out = out + [ obj.Var(i, :, :) >= obj.Min(i) ];
        end
    end
else
    % parametric bounds
    out = out + [ obj.Var >= obj.Internal.parameters.Min ];
end
