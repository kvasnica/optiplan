classdef Obstacle < moantool.utils.OMPBaseClass
    ...
    methods
        ...
        function obj = Obstacle(agent, nObstacles)
            ...
            if nObstacles>1
                obj = [];
                for i = 1:nObstacles
                    obj = [obj, moantool.Obstacle(agent)];
                end
                return
            end
            
            % position of the obstacle
            obj.Position = moantool.AgentSignal([agent.ny 1], agent.N, 'Position');
            % whether the obstacle is visible
            obj.Visible = moantool.AgentSignal([1 1], agent.N, 'Visible');
            % size along the axis
            obj.Size = moantool.AgentSignal([agent.ny 1], agent.N, 'Size');
            % remove all filters
